package app.temp.red.colorarcprogressbar;
...
public class ColorArcProgressBar extends View{
    ...
    //上限与下限
    private float curValuesMax=100;
    private float curValuesMin=0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(String.format("%.0f",curValuesMin+(curValuesMax-curValuesMin)*curValues/100), centerX, centerY + textSize / 3, vTextPaint);
        }
        if (isNeedUnit) {
    ...
    public void setNumRange(float min,float max){
        this.curValuesMin=min;
        this.curValuesMax=max;
    }
